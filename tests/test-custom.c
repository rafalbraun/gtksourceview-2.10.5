...
GtkTreeIter append_node(GtkTreeStore *treestore, GtkTreeIter* parent, gchar* node_name);
GtkTreeIter append_node(GtkTreeStore *treestore, GtkTreeIter* parent, gchar* node_name);
GtkWidget *create_view_and_model(char* path);
GtkWidget* create_gtk_tree_view(char* path);
GtkTreeModel *create_and_fill_model_fs(char* path);
void read_all_files (GtkTreeStore* treestore, const gchar* path, GtkTreeIter* parent);
gboolean view_onButtonPressed(GtkWidget *treeview, GdkEventButton *event, gpointer userdata);
gboolean view_onKeyPressed(GtkWidget *view, GdkEventKey *event, gpointer userdata);
...
on_notebook_switch_page (GtkNotebook *notebook,
    ...
    printf("%d , %d, %s\n", gtk_notebook_page_num(notebook, child), gtk_notebook_page_num(notebook, page), gtk_notebook_get_tab_label_text(notebook, child));
}
...
static GtkActionEntry view_action_entries[] = {
	...
	{ "Aaaa", NULL, "aaa", "<control><shift>F",
	  "aaa", G_CALLBACK (aaa_cb) },
...
void init_list(GtkWidget *list) {
  ...
  column1 = gtk_tree_view_column_new_with_attributes("List Items", renderer, "text", LIST_ITEM, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(list), column1);
  column2 = gtk_tree_view_column_new_with_attributes("Item Names", renderer, "text", ITEM_NAME, NULL);
...
void add_to_list(GtkWidget *list, const gchar *str1, const gchar *str2) {
  ...
  gtk_list_store_set(store, &iter, LIST_ITEM, str1, ITEM_NAME, str2, -1);
}
...
aaa_cb ()
{
	...
	GtkTextBuffer* buffer = gtk_source_buffer_new (NULL);
	gtk_source_buffer_load_file (buffer, "/home/vanqyard/xd.cpp", NULL);
	...
	*/
	...
	gtk_container_add (GTK_CONTAINER (scroll), view);
  	...
  	init_list(list);
	add_to_list(list, "Aliens", "Aliens");
	add_to_list(list, "Leon",   "Leon");
	add_to_list(list, "The Verdict", "2dqw");
	add_to_list(list, "North Face", "efwef");
	add_to_list(list, "Der Untergang", "ewfwef");

	//GtkWidget* btn = gtk_button_new_with_label("Quit");

	gtk_paned_pack1(GTK_PANED(vbox), scroll, TRUE, TRUE);
	gtk_paned_pack2(GTK_PANED(vbox), list, FALSE, FALSE);
	gtk_container_set_border_width(GTK_CONTAINER(vbox), 15);
    gtk_paned_set_position ((GtkPaned*)vbox, 600);
...
GtkWidget* prepare_view(GtkWidget* view, GtkSourceBuffer* buffer, GtkWidget* window) {
	return view;
}
...
create_view_window (GtkSourceBuffer *buffer, GtkSourceView *from)
{
	...
	*/
    ...
    gtk_paned_set_position ((GtkPaned*)hbox, 140);
    ...
    GtkWidget* tree = create_gtk_tree_view("/home/vanqyard/Pulpit/gtkwrite");
	gtk_paned_pack1 (GTK_PANED (hbox), tree, FALSE, FALSE );
	gtk_widget_show(tree);
	gtk_widget_show(hbox);

	gtk_paned_set_position (hbox,
    	                    360);

	GtkWidget* notebook = gtk_notebook_new ();

	gtk_paned_pack2 (GTK_PANED (hbox), notebook, TRUE, TRUE );
	gtk_container_add (GTK_CONTAINER (sw), view);
	//gtk_box_pack_start (GTK_BOX (vbox), pos_label, FALSE, FALSE, 0);
	
	GtkWidget* label = gtk_label_new ("Untitled");
	gtk_notebook_append_page ( GTK_NOTEBOOK(notebook), sw, GTK_LABEL(label));
...
GtkTreeIter append_node(GtkTreeStore *treestore, GtkTreeIter* parent, gchar* node_name) {
  ...
  gtk_tree_store_set(treestore, &child, COLUMN, node_name, -1);
...
GtkWidget *create_view_and_model(char* path) {
  ...
  gtk_tree_view_column_set_title(col, "[AwesomeProject]");
  ...
  model = create_and_fill_model_fs(path);
  gtk_tree_view_set_model(GTK_TREE_VIEW(view), model);
  g_object_unref(model); 

  g_signal_connect(view, "button-press-event", (GCallback) view_onButtonPressed, NULL);
  //g_signal_connect (G_OBJECT (window), "key_press_event", G_CALLBACK (view_onKeyPressed), NULL);
  g_signal_connect(view, "key_press_event", G_CALLBACK (view_onKeyPressed), NULL);
...
GtkWidget* create_gtk_tree_view(char* path) {
    ...
    GtkWidget* view;
    view = create_view_and_model(path);
    ...
    // gtk_window_set_default_size(GTK_SCROLLED_WINDOW(scrolled_window), 400, 300);
    // gtk_window_set_resizable (GTK_SCROLLED_WINDOW(scrolled_window), TRUE);
...
GtkTreeModel *create_and_fill_model_fs(char* path) {
  ...
  GtkTreeIter root = append_node(treestore, NULL, path);
  read_all_files(treestore, path, &root);

  return GTK_TREE_MODEL(treestore);
}

void
read_all_files (GtkTreeStore* treestore, const gchar* path, GtkTreeIter* parent)
{
    const gchar *file;
    GDir *dir;
    gchar *newpath;
    ...
    while ((file = g_dir_read_name (dir)))
    {
        ...
        {
            GtkTreeIter child = append_node(treestore, parent, fullpath + strlen(path) +1); 
            //g_print ("d: %s was opened \n", fullpath);
            //print(fullpath);
            
            read_all_files (treestore, fullpath, &child);

        } else if (g_file_test (fullpath, G_FILE_TEST_IS_REGULAR)) {

            append_node(treestore, parent, fullpath + strlen(path) +1); 
            //g_print ("f: %s was opened \n", fullpath);
            //print(fullpath);

        } else {
            g_print ("???: %s was opened ", fullpath);
        }
        g_free(fullpath);
    }
    g_dir_close (dir);
...
gboolean
view_onKeyPressed() {
	GdkEventButton* event = (GdkEventButton*)eventAAA;
	GdkEventKey* eventKey = (GdkEventKey*)eventAAA;

	// if( eventKey->keyval == GDK_KEY_F &&
...
*/
gboolean
view_onKeyPressed(GtkWidget *view, GdkEventKey *event, gpointer userdata) {
...
view_onButtonPressed(GtkWidget *treeview, GdkEventButton *event, gpointer userdata)
{
  ...
  if (event->type == GDK_2BUTTON_PRESS) {
      ...
      if (gtk_tree_selection_count_selected_rows(selection) == 1) {
        ...
        //GtkTreePath *xxx;
        //gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(treeview), event->x, event->y, &xxx, NULL, NULL, NULL);
        //check_if_exists(GTK_TREE_STORE(model), xxx);
